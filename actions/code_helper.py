...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"misha_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"misha_debug_{int(time.time())}.png"
...
Code:"""
    ...
    code = _clean_code(generate_text(prompt, temperature=0.1))
...
Fixed code:"""
    ...
    return _clean_code(generate_text(prompt, temperature=0.1))
...
Updated code:"""
    ...
    try:
        edited = _clean_code(generate_text(prompt, temperature=0.1))
    except Exception as e:
...
Explanation:"""
    ...
    try:
        return generate_text(prompt, temperature=0.2).strip()
    except Exception as e:
...
Optimized code:"""
    ...
    try:
        optimized = _clean_code(generate_text(prompt, temperature=0.1))
    except Exception as e:
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    return (
        "Local screenshot analysis is not enabled yet. "
        "Misha will require a separate on-device vision model; no cloud API was called."
    )
