...
def _get_os() -> str:
    detected = {"Darwin": "mac", "Windows": "windows", "Linux": "linux"}.get(platform.system(), "unknown")
    try:
        from memory.config_manager import get_config
        configured = get_config("os_system")
    except Exception:
        configured = None
    return (configured or detected).lower()
...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = Path.home() / "Desktop" / "misha_screenshot.png"
    if not requested:
        return fallback
    try:
        p = Path(requested).expanduser().resolve()
        for root in _SAFE_SCREENSHOT_ROOTS:
            if p.is_relative_to(root.resolve()) and not p.is_symlink():
...
def _user_profile() -> dict:
    ...
    try:
        ...
        return {
            key: entry.get("value", "") if isinstance(entry, dict) else str(entry)
...
def _clipboard_get() -> str:
    ...
    _hotkey("command" if _get_os() == "mac" else "ctrl", "c")
    time.sleep(0.2)
...
def _clear_field() -> str:
    _require_pyautogui()
    pyautogui.hotkey("command" if _get_os() == "mac" else "ctrl", "a")
    time.sleep(0.1)
...
def _focus_window(title: str) -> str:
    os_name = _get_os()
    try:
        title = safe_window_title(title)
    except ValueError as exc:
        return f"focus_window blocked: {exc}"
...
def _screen_find(description: str) -> tuple[int, int] | None:
    print(
        "[ComputerControl] Local coordinate vision is not configured; "
        f"cannot locate {description!r}."
    )
    return None
