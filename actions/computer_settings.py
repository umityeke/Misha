...
from core.system_capabilities import format_capabilities
...
def volume_mute(muted: bool = True):
    if _OS == "Windows":
        try:
            ...
            raise RuntimeError("Exact mute control is unavailable on Windows.") from exc
    elif _OS == "Darwin":
...
def read_audio_state() -> dict[str, int | bool]:
    """Read the default output state independently from the mutation path."""
    if _OS == "Darwin":
        script = (
        ...
        volume_text, muted_text = result.stdout.strip().split("|", 1)
        return {"volume": int(volume_text), "muted": muted_text.casefold() == "true"}
    if _OS == "Linux":
        ...
        if match is None:
            raise RuntimeError("Default sink volume could not be parsed.")
        return {
            "volume": int(match.group(1)),
            "muted": mute_result.stdout.rsplit(":", 1)[-1].strip().casefold() == "yes",
        }
    if _OS == "Windows":
        from ctypes import cast, POINTER
        from comtypes import CLSCTX_ALL
        ...
        endpoint = cast(interface, POINTER(IAudioEndpointVolume))
        return {
            "volume": round(float(endpoint.GetMasterVolumeLevelScalar()) * 100),
            "muted": bool(endpoint.GetMute()),
        }
    raise RuntimeError(f"Audio-state verification is unsupported on {_OS}.")
...
def brightness_up():
    if _OS == "Darwin":
        ...
        if subprocess.run(["which", "brightnessctl"], capture_output=True).returncode == 0:
            ...
            raise RuntimeError("Brightness control requires brightnessctl; shell fallback is disabled.")
    else:
        try:
            subprocess.run(
                ["powershell", "-Command",
...
def computer_settings(
    ...
    if action in {"capabilities", "capability_matrix"}:
        return format_capabilities(_OS, pyautogui_available=_PYAUTOGUI)
    ...
    if action in _DANGEROUS_ACTIONS:
        confirmed = " ".join(str(params.get("confirmed", "")).upper().split())
        required = f"CONFIRM {action.upper()}"
        if confirmed != required:
            return (
                f"This will {action} the computer. "
                f"A separate runtime approval and exact confirmation '{required}' are required."
            )
