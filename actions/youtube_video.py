...
def _is_valid_youtube_url(url: str) -> bool:
    ...
    return (
        parsed.scheme.casefold() in {"https", "http"}
        and host in allowed
...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("M.I.S.H.A", prompt_text, parent=root)
...
def _summarize_locally(transcript: str, video_url: str) -> str:
    ...
    return generate_text(
        ...
        system=(
            "You are MISHA. Summarize clearly with a one-sentence overview and "
            "3-5 key points. Match the transcript language."
        ),
        temperature=0.2,
    )
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    url = str(parameters.get("url", "")).strip() or _ask_for_url(
        "Please paste the YouTube video URL:"
    )
    if not url:
    ...
    try:
        summary = _summarize_locally(transcript, url)
    except Exception as e:
