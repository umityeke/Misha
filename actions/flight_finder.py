...
def _parse_date(raw: str) -> str:
    ...
    raise ValueError("Flight date could not be parsed safely.")
...
def _parse_flights_locally(
    ...
    try:
        flights = generate_json(
            prompt,
            system="Extract flight information and return only valid JSON.",
            temperature=0.0,
        )
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    passengers  = max(1, min(9, int(params.get("passengers", 1))))
    ...
    if any(len(value) > 80 or not re.fullmatch(r"[\w .()'’-]+", value, re.UNICODE)
           for value in (origin, destination)):
        return "Flight origin or destination is invalid."
    ...
    try:
        ...
        return "Flight date is invalid. Use YYYY-MM-DD or an unambiguous date."
    ...
    try:
        ...
        flights = _parse_flights_locally(raw_text, origin, destination, date)
