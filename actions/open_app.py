...
def _launch_windows(app_name: str) -> bool:
    ...
    try:
        parts = shlex.split(app_name, posix=False)
    except ValueError:
        return False
...
def _launch_linux(app_name: str) -> bool:
    ...
    binary = (
        shutil.which(first) or shutil.which(first.lower())
        or shutil.which(first.lower().replace(" ", "-"))
        or shutil.which(first.lower().replace(" ", "_"))
    )
    if binary:
        try:
            subprocess.Popen(
