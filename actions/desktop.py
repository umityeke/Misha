...
def _execute_generated_code(code: str, player=None) -> str:
    return "Generated desktop code execution is disabled; use a registered action."


def _ask_local_model_for_desktop_action(task: str) -> str:
...
Task: {task}"""
    ...
    try:
        code = generate_text(prompt, temperature=0.1).strip()
        if code.startswith("```"):
            lines = code.split("\n")
        ...
        if _OS == "Windows":
            ...
            if path.suffix.lower() in {".webp", ".png"}:
                try:
                    ...
                    descriptor, temporary_name = tempfile.mkstemp(suffix=".bmp")
                    os.close(descriptor)
                    bmp_path = Path(temporary_name)
            ...
            subprocess.run(["osascript", "-e", script, str(path)], capture_output=True)
            return f"Wallpaper set: {path.name}"
...
def set_wallpaper_from_url(url: str) -> str:
    return (
        "Wallpaper URL download is disabled to prevent redirect/SSRF risks. "
        "Download an image with the guarded browser tool, then choose the local file."
    )
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            return (
                "Free-form generated desktop actions are disabled for safety. "
                "Use a registered desktop action."
            )

        else:
            return f"Unknown or disabled desktop action: {action or '(empty)'}"
