...
class TaskQueue:
    ...
    def _run_task(self, task: Task) -> None:
        ...
        try:
            ...
            with self._lock:
                if result.status is ResultStatus.CANCELLED or task.cancel_flag.is_set():
                    task.status = TaskStatus.CANCELLED
            ...
            if task.on_complete and result.succeeded and not task.cancel_flag.is_set():
                try:
                    task.on_complete(task.task_id, result)
                except Exception as e:
                    print(f"[TaskQueue] ⚠️ on_complete callback error: {e}")

            print(f"[TaskQueue] {result.status.value}: [{task.task_id}]")
